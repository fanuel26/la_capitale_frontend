...
export class DetailProduitComponent implements OnInit, OnChanges {
  @Input() id!: number;
  detailProduit: any;
  prixVente: any;
  lotProduit: any;
  ...
  onSubmitLot() {
    ...
    this.produitService.saveLotProduit(body).subscribe(value => {
      if (value.status == true) {
        this.getDetailProduit(this.id);
        this.lotForm.reset()
        this.sweetAlert.showSuccessAlert('Opération effectuer!', 'Mise en stock effectuer avec avec success')
      }
    })
  }
    ...
    if (this.prixVente == null) {
      ...
      this.produitService.savePrixVenteProduit(body).subscribe(value => {
        if (value.status == true) {
          ...
          this.sweetAlert.showSuccessAlert('Opération effectuer!', 'Prix de vente renseigner avec success')
        }
      })
    } else {
      let body = {
      ...
      this.produitService.updatePrixVenteProduit(this.prixVente.id, body).subscribe(value => {
        if (value.status == true) {
          this.getDetailProduit(this.id);
          this.venteForm.reset()
          this.sweetAlert.showSuccessAlert('Opération effectuer!', 'Prix de vente mise à jour avec success')
        }
      })
    }
  }
  ...
  getDetailProduit(id: number) {
    ...
    this.produitService.getStatProduit(id).subscribe(value => {
      if (value.status == true) {
        ...
        this.nbrStock = this.stateProduit.nbrQte ?? 0 - this.stateProduit.nbrQteVente ?? 0
      }
    })
  }
}
